---
name: release
description: Cut a release of the cmem-package-template repository itself - verify preconditions, rename the changelog heading, commit, sign the tag and push. Use when asked to release, tag or publish a new version of this template.
---

# Release procedure for cmem-package-template

This describes how **this repository** is released. It does not apply to
packages generated from the template — those are published to a Corporate
Memory deployment with `cmemc package install` and derive their version from
`git describe`, with no branching convention imposed by the template.
...
points at *is* the release: `copier copy gh:eccenca/cmem-package-template`
resolves the newest tag, so tagging **is** publishing. Never create a GitHub
Release.

This procedure is deliberately identical to the one in the sibling repository
`eccenca/cmem-plugin-template`. If you improve something here, consider whether
it belongs there too, and read that repository's copy before designing anything
new.

Accepts an optional version argument, e.g. `/release 1.5.0`. Without it, derive
...
Before 1.5.0 this repository merged `develop` into `main` with `--no-ff`, which
left merge commits on `main` and made the fast-forward impossible. A one-time
merge of `main` into `develop` reconciled the two, and `main` has been a plain
pointer since.
...
7. **Changelog is ready.** `CHANGELOG.md` contains a `## [Unreleased]` heading
   ...
   Then check that it actually *covers* the user-visible changes. `copier.yaml`
   sets `_subdirectory: src`, so everything under `src/` is rendered into
   generated packages and anything listed by this needs a changelog entry:

   ```bash
   git diff --name-only "$(git describe --tags --abbrev=0)"..develop -- src/
   ```

   Changes *outside* `src/` — this repository's own `Taskfile.yaml`, `tests/`,
   `.github/`, or this skill — are template-internal and are normally not
   listed at all. The exception is the cmemc pin in
   `.github/workflows/check.yml`, which is bumped in lockstep with the one in
   `src/` and is named in the entry the `src/` change earns.

8. **Signing works.** Verify gpg can sign non-interactively *now*, rather than
...
9. **CI is green on exactly this commit.**
   ...
   This is the real gate on the release. The tag is pushed without waiting for
   the run on `main` — see the caveat at the end for why that is sound.

10. **No other CI run is occupying the concurrency group.**
    ...
    cancelled. This is what painted `main` red on 2026-09-03: the run on the
    release merge was cancelled by the next push, not failed.

    The group is not merely a formality here. `task check` installs and
    uninstalls packages on a **live, shared** Corporate Memory deployment, so
    two concurrent runs corrupt each other's state rather than just queueing.

    If a run is in flight, stop and say which branch holds the group.
...
- **Patch** (`X.Y.Z+1`) — the section contains *only* `### Fixed`.
  Precedent: 1.2.2, 1.2.1, 0.9.7, 0.5.1.
- **Major** (`X+1.0.0`) — an existing generated package cannot take the new
  template with a plain `copier update`: a copier question removed or renamed,
  a `package_id` regex change that invalidates answers already recorded in
  `.copier-answers.yml`, or a restructure of the generated layout that forces
  manual reconciliation. There is no precedent in the 1.x line; 1.0.0 was a
  stability milestone rather than a breaking change.
- **Minor** (`X.Y+1.0`) — everything else, including new copier questions,
  cmemc bumps, and changes to the generated pipeline or manifest. This covers
  the large majority of releases. Precedent: 1.4.0, 1.3.0, 1.2.0.

Judgement calls belong to the user. A change that *might* disturb `copier
update` for some answer combinations is arguably breaking but has historically
been a minor — say so and let them decide rather than choosing silently.
...
passed explicitly. Note the subject is `release X.Y.Z` — releases before 1.5.0
used `update CHANGELOG`, which does not distinguish a release from an ordinary
changelog edit.
...
The old procedure pushed three times — `update CHANGELOG`, the merge to `main`,
then `add unreleased section` — which is precisely why the run on `main` kept
being cancelled.
...
release get built — which is the reason preflight check 10 exists, since a
